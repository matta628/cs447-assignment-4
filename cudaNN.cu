...
FullyConnectedLayer<IN_DIMS, N_NEURONS>::FullyConnectedLayer(const std::string &n, const bool relu, const double do_rate, const int seed_seq)
 : m_name(n), m_relu(relu), m_keep_prob(1 - do_rate), m_all_kept(1), m_eng(7389 + seed_seq), block_size(128) {
    ...
    cudaError_t rv_ce;
    int H32 = (IN_DIMS::H == 1) ? 1 : 32;
    int W32 = (IN_DIMS::W % 32 == 0)? IN_DIMS::W : 32;
    int RDHW = IN_DIMS::D*H32*W32;
    int NRDHW = N_NEURONS*RDHW;

    //  FullyConnectedLayer<Dims<1, 28, 28>, 1024>
    //  FullyConnectedLayer<Dims<1, 1, 1024>, 10>
    //  true_h, true_w
    ...
    gpu_assert(rv_ce);
    rv_ce = cudaMalloc(&d_downstream_deriv, RDHW*sizeof(double));
    gpu_assert(rv_ce);
    rv_ce = cudaMalloc(&d_upstream_deriv, N_NEURONS*sizeof(double));
    gpu_assert(rv_ce);
    rv_ce = cudaMalloc(&d_weight, RNDHW*sizeof(double));
    gpu_assert(rv_ce);
    rv_ce = cudaMalloc(&d_weight_deriv, RNDHW*sizeof(double));
    gpu_assert(rv_ce);
    rv_ce = cudaMalloc(&d_input, RDHW*sizeof(double));
    gpu_assert(rv_ce);
    rv_ce = cudaMalloc(&d_bias_deriv, N_NEURONS*sizeof(double));
    gpu_assert(rv_ce);
    rv_ce = cudaMalloc(&d_bias, N_NEURONS*sizeof(double));
    gpu_assert(rv_ce);
    fflush("constructor ends\n");
}
...
FullyConnectedLayer<IN_DIMS, N_NEURONS>::backprop(const Output &full_upstream_deriv, const double mb_size) {
    ...
    int W32 = (IN_DIMS::W % 32 == 0)? IN_DIMS::W : 32;
    int RDHW = IN_DIMS::D*H32*W32;
    int RNDHW = RDHW*N_NEURONS;

    //reshape weight and weight_deriv
    Array<Array<Precision,Dims<IN_DIMS::D,H32,W32>,N_NEURONS> reshape_w;
    Array<Array<Precision,Dims<IN_DIMS::D,H32,W32>,N_NEURONS> reshape_w_d;
    for (int n = 0; n < N_NEURONS; n++){
        reshape[n] = 0;
        for (int h = 0; h < H32; h++){
            for (int w = 0; w < W32; w++){
                if (h < IN_DIMS::H && w < IN_DIMS::W){
                    reshape_w[n][0][h][w] = this->m_weight[n][0][h][w];
                    reshape_w_d[n][0][h][w] = this->m_weight_deriv[n][0][h][w];
                }
            }
        }
    }

    //reshape downstream_deriv and input
    Array<Precision,Dims<IN_DIMS::D,H32,W32> reshape_dd = 0;
    Array<Precision,Dims<IN_DIMS::D,H32,W32> reshape_i = 0;
    for (int h = 0; h < H32; h++){
        for (int w = 0; w < W32; w++){
            if (h < IN_DIMS::H && w < IN_DIMS::W){
                reshape_dd[0][h][w] = this->downstream_deriv[0][h][w];
                reshape_i[0][h][w] = input[0][h][w];
            }
        }
    }
    ...
    rv_ce = cudaMemcpy(d_downstream_deriv, &reshape_dd, RDHW*sizeof(double), cudaMemcpyHostToDevice);
    gpu_assert(rv_ce);

    //RESHAPE
    //Array<D,H,W> input
    rv_ce = cudaMemcpy(d_input, &reshape_i, RDHW*sizeof(double), cudaMemcpyHostToDevice);
    gpu_assert(rv_ce);


    //RESHAPE
    //Array<Array<D,H,W>, N_NEURON> m_weight
    rv_ce = cudaMemcpy(d_weight, &reshape_w, RNDHW*sizeof(double), cudaMemcpyHostToDevice);
    gpu_assert(rv_ce);

    //RESHAPE
    //Array<Array<D,H,W>, N_NEURON> m_weight_deriv
    rv_ce = cudaMemcpy(d_weight_deriv, &reshape_w_d, RNDHW*sizeof(double), cudaMemcpyHostToDevice);
    gpu_assert(rv_ce);
    ...
    int grid_size = (RNDHW + block_size - 1)/ block_size;
    ...
    backprop_dev<<<grid_size,block_size>>>(d_current_kept, m_relu, d_output, d_downstream_deriv,
        ...
        N_NEURONS,IN_DIMS::D,H32,W32);

    //TODO: copy device mem to host
    //downstream
    rv_ce = cudaMemcpy(&reshape_dd, d_downstream_deriv, RDHW*sizeof(double), cudaMemcpyDeviceToHost);
    gpu_assert(rv_ce);

    //reshape downstream back
    for (int h = 0; h < IN_DIMS::H; h++){
        for (int w = 0; w < IN_DIMS::W; w++){
            this->downstream_deriv[0][h][w] = reshape_dd[0][h][w];
        }
    }

    //weight_deriv
    rv_ce = cudaMemcpy(&reshape_w_d, d_weight_deriv, RNDHW*sizeof(double), cudaMemcpyDeviceToHost);
    gpu_assert(rv_ce);

    //reshape weight_deriv back
    for (int n = 0; n < N_NEURONS; n++){
        for (int h = 0; h < IN_DIMS::H; h++){
            for (int w = 0; w < IN_DIMS::W; w++){
                this->m_weight_deriv[n][0][h][w] = reshape_w_d[n][0][h][w];
            }
        }
    }
...
FullyConnectedLayer<IN_DIMS, N_NEURONS>::update_weights(const float rate) {
    ...
    //TODO: actually implement kernel
    update_weights_dev<<<grid_size,block_size>>>(d_weight, rate, d_weight_deriv, d_bias, d_bias_deriv, RDHW);

    //TODO: copy device mem to host
    //weight
    rv_ce = cudaMemcpy(&reshape_w, d_weight, RNDHW*sizeof(double), cudaMemcpyDeviceToHost);
    gpu_assert(rv_ce);
    //weight_deriv
    rv_ce = cudaMemcpy(&reshape_w_d, d_weight_deriv, RNDHW*sizeof(double), cudaMemcpyDeviceToHost);
    gpu_assert(rv_ce);

    //reshape weight and weight_deriv back
    for (int n = 0; n < N_NEURONS; n++){
        for (int h = 0; h < IN_DIMS::H; h++){
            for (int w = 0; w < IN_DIMS::W; w++){
                this->m_weight[n][0][h][w] = reshape_w[n][0][h][w];
                this->m_weight_deriv[n][0][h][w] = reshape_w_d[n][0][h][w];
            }
        }
    }
